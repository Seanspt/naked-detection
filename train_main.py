...
from input import extract_data, IMAGE_SIZE_HEIGHT, IMAGE_SIZE_WIDTH
...
class Dataset(object):
    ...
    def read(self, img_rows=IMAGE_SIZE_HEIGHT, img_cols=IMAGE_SIZE_WIDTH, img_channels=3, nb_classes=2):
        ...
        X_valid, _, y_valid, _ = train_test_split(images, labels, test_size=0.5, random_state=random.randint(0, 100))
...
class Model(object):
    ...
    def evaluate(self, dataset):
        y_true = dataset.Y_test[:, 1].astype('int32')
        y_proba_predicted = (self.model.predict(dataset.X_test) > 0.5).astype('int32')[:, 1]

        for t in (0.5, 0.6, 0.7, 0.8, 0.9):
            print 't:', t
            y_predicted = (y_proba_predicted > t).astype('int32')[:, 1]
